"""
Run the head EWC checkpoint on the head OOD set only.

This mirrors `eval_head_only_batch.py` but defaults to:
  - the EWC checkpoint at 768 resolution
  - evaluating only the `ood` split under `eval_samples_805/heads`
  - saving videos under `out/heads_ewc_ood_eval`
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Root directory containing head splits (expects an `ood` folder).",
    )
    parser.add_argument(
        "--output_dir",
        default="out/heads_ewc_ood_eval",
        help="Directory to store rendered OOD videos.",
    )
    parser.add_argument(
        "--lora_path",
        default="/home/longnhat/Lin_workspace/8TB2/Lin/801_Project/805/DiffSynth-Studio/models/train/805_head_with_body_guard_768/epoch-3.safetensors",
        help="Path to the head EWC (body guard) checkpoint.",
    )
    ...
    parser.add_argument("--quality", type=int, default=5, help="FFMPEG quality (1-31, lower is better).")
    ...
    parser.add_argument(
        "--tiled",
        action="store_true",
        help="Enable tiled generation (helps with VRAM usage).",
    )
    parser.add_argument(
        "--splits",
        nargs="+",
...
def main() -> None:
    ...
    print(f"[INFO] Loaded EWC checkpoint: {args.lora_path}")
    ...
    if args.include_file:
        ...
        if not include_file.is_file():
            raise SystemExit(f"include_file not found: {include_file}")
