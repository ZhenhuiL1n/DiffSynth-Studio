#!/usr/bin/env python3
"""
Evaluate the mixed baseline results (body/head) against GT videos.

Computes PSNR/SSIM/LPIPS/CSIM for the `in_dist` folders after resizing frames
to 758x758 by default.
"""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Evaluate baseline_results/mixed body/head splits vs GT.")
    ap.add_argument(
        "--root",
        default="/home/longnhat/Lin_workspace/8TB2/Lin/801_Project/805/DiffSynth-Studio/baseline_results",
        help="Root directory containing `mixed` and `GT` subfolders.",
    )
...
def main() -> None:
    ...
    gt_root = root / "GT"
    ensure_dir(mixed_root, "mixed root")
