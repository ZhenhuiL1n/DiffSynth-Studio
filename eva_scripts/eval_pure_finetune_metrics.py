#!/usr/bin/env python3
"""
Compute PSNR/SSIM/LPIPS/CSIM for the pure fine-tune experiments (head/body) using GT videos.

Only `in_dist` folders are evaluated and frames are resized to 758x758 by default.
"""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Evaluate pure fine-tune experiments (body/head) against GT videos.")
    ap.add_argument(
        "--root",
        default="/home/longnhat/Lin_workspace/8TB2/Lin/801_Project/805/DiffSynth-Studio/baseline_results",
        help="Root directory containing `pure_fintune` and `GT` subfolders.",
    )
    ...
    ap.add_argument("--json_output", default=None, help="Optional JSON file to store aggregated metrics.")
...
def main() -> None:
    args = parse_args()
    root = Path(args.root)
    pure_root = root / "pure_fintune"
    gt_root = root / "GT"
    ensure_dir(pure_root, "pure_fintune root")
