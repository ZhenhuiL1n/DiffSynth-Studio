#!/usr/bin/env python3
"""
Evaluate the sequential fine-tuning (EWC) experiments against GT videos.

Computes PSNR/SSIM/LPIPS/CSIM for the `in_dist` body/head folders after resizing
frames to 758x758 by default.
"""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Evaluate EWC seq_finetune body/head splits vs GT.")
    ap.add_argument(
        "--root",
        default="/home/longnhat/Lin_workspace/8TB2/Lin/801_Project/805/DiffSynth-Studio/EWC_experiment",
        help="Root directory containing `seq_finetune` and `GT` subfolders.",
    )
...
def main() -> None:
    args = parse_args()
    root = Path(args.root)
    seq_root = root / "seq_finetune"
    gt_root = root / "GT"
    ensure_dir(seq_root, "seq_finetune root")
