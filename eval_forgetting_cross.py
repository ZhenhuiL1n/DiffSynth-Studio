"""
Cross-evaluation script to measure forgetting after cross-task finetuning.

Runs two evaluations:
1) Head dataset using the checkpoint finetuned on body (head_to_body run).
2) Body dataset using the checkpoint finetuned on head (body_to_head run).

For each, it iterates both in_dist and ood splits (override with --splits) and
writes outputs to separate folders.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    # Head eval (checkpoint finetuned on body)
    parser.add_argument(
        "--head_lora",
        default="/home/longnhat/Lin_workspace/8TB2/Lin/801_Project/805/DiffSynth-Studio/models/train/805_head_to_body/epoch-3.safetensors",
        help="LoRA checkpoint path for evaluating the head dataset (head->body finetune).",
    )
    parser.add_argument(
        "--head_input_root",
        default="/home/longnhat/Lin_workspace/8TB2/Lin/801_Project/805/DiffSynth-Studio/eval_samples_805/heads",
        help="Root containing head evaluation images (subfolders like in_dist, ood).",
    )
    parser.add_argument(
        "--head_output_root",
        default="out/forgetting/head_eval_on_head_to_body",
        help="Where to save head evaluation videos (split subfolders will be created).",
    )

    # Body eval (checkpoint finetuned on head)
    parser.add_argument(
        "--body_lora",
        default="/home/longnhat/Lin_workspace/8TB2/Lin/801_Project/805/DiffSynth-Studio/models/train/805_body_to_head/epoch-3.safetensors",
        help="LoRA checkpoint path for evaluating the body dataset (body->head finetune).",
    )
    parser.add_argument(
        "--body_input_root",
        default="/home/longnhat/Lin_workspace/8TB2/Lin/801_Project/805/DiffSynth-Studio/eval_samples_805/bodys",
        help="Root containing body evaluation images (subfolders like in_dist, ood).",
    )
    parser.add_argument(
        "--body_output_root",
        default="out/forgetting/body_eval_on_body_to_head",
...
def run_eval(
    ...
    pipe = build_pipeline(args.device, lora_path, args.lora_alpha)
    print(f"[{name}] Loaded pipeline with LoRA: {lora_path}")
...
def main() -> None:
    ...
    # Head eval on head->body checkpoint (measure head forgetting)
    run_eval(
        name="head_eval_on_head_to_body",
        lora_path=args.head_lora,
    ...
    # Body eval on body->head checkpoint (measure body forgetting)
    run_eval(
        name="body_eval_on_body_to_head",
        lora_path=args.body_lora,
