...
# Evaluate a full-finetuned FLUX.2-klein checkpoint using image+prompt pairs from metadata.csv.
#
# Default behavior:
# - runs full-checkpoint inference on all rows in metadata
# - saves outputs under OUT_ROOT/full/<relative_image_path>
#
# Optional:
...
METADATA_PATH="${METADATA_PATH:-${ROOT_DIR}/data/back_view_dataset/metadata_full_0008_01_to_0133_07.csv}"
MODEL_ID="${MODEL_ID:-black-forest-labs/FLUX.2-klein-4B}"
FULL_CHECKPOINT="${FULL_CHECKPOINT:-${ROOT_DIR}/models/train/FLUX.2-klein-4B-backview-full-0008_01_to_0133_07/epoch-4.safetensors}"
OUT_ROOT="${OUT_ROOT:-${ROOT_DIR}/outputs/eval_backview_full_epoch4}"
...
if [[ ! -f "${FULL_CHECKPOINT}" ]]; then
  echo "Full checkpoint not found: ${FULL_CHECKPOINT}" >&2
  exit 1
fi

mkdir -p "${OUT_ROOT}/full"
if [[ "${COMPARE_BASE}" == "true" ]]; then
  mkdir -p "${OUT_ROOT}/base"
fi
cp -f "${METADATA_PATH}" "${OUT_ROOT}/metadata_snapshot.csv"

rows_tsv="$(mktemp /tmp/backview_eval_full_rows.XXXXXX.tsv)"
...
echo "Running full-checkpoint evaluation"
...
while IFS=$'\t' read -r rel_image prompt; do
  ...
  out_full="${OUT_ROOT}/full/${rel_image}"
  ...
  if [[ "${SKIP_EXISTING}" != "true" || ! -f "${out_full}" ]]; then
    echo "[${done_count}/${total}] Full -> ${rel_image}"
    "${PYTHON_BIN}" "${INFER_SCRIPT}" \
      --input_image "${input_image}" \
      --prompt "${prompt}" \
      --output "${out_full}" \
      --model_id "${MODEL_ID}" \
      --dit_checkpoint "${FULL_CHECKPOINT}" \
      --device "${DEVICE}" \
      --mode "${MODE}" \
