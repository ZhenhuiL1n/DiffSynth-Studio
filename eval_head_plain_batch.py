"""
Batch-evaluate the plain head fine-tuning checkpoint on the evaluation set.

This mirrors ``eval_head_only_batch.py`` but defaults to the user's plain
checkpoint at 768 resolution.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to the 768px head LoRA checkpoint.",
    )
...
def load_image_fit(source: Path, target_w: int, target_h: int, pad_color=(0, 0, 0)) -> Image.Image:
    """Load an image, respect EXIF orientation, pad/resize to target size."""
