"""
Batch-evaluate a head-only LoRA checkpoint on all images in a directory.

Defaults match the user's head-only training setup (512x512, 49 frames) and
consume images from ``eval_samples_805/heads/in_dist``. Override paths and
prompts via CLI flags as needed.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output_dir",
        default="out/heads_eval",
        help="Root directory for rendered videos (split subfolders will be created).",
    )
    parser.add_argument(
        "--lora_path",
        default="/home/longnhat/Lin_workspace/8TB2/Lin/801_Project/805/DiffSynth-Studio/models/train/805_baseline_headonly/epoch-8.safetensors",
        help="Path to the head-only LoRA checkpoint.",
    )
    ...
    parser.add_argument(
        "--include_file",
        default=None,
        help="Optional text file (one stem per line) specifying inputs.",
    )
    parser.add_argument(
        "--skip_existing",
        action="store_true",
...
def load_image_fit(source: Path, target_w: int, target_h: int, pad_color=(0, 0, 0)) -> Image.Image:
    """Load an image from disk or URL, respect EXIF orientation, pad/resize to target size."""
    ...
    for split in args.splits:
        ...
        for idx, img_path in enumerate(images):
            ...
            video = pipe(
                ...
                seed=args.seed + idx,  # vary seed per sample for minor diversity
                tiled=args.tiled,
            )
